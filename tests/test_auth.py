...
TEST_ENV = {"NANOBANANA_ENV_PATH": "/tmp/image-this-mcp-test-does-not-exist.env"}
...
class TestAuthConfiguration:
    ...
    def test_auto_allows_openai_only_configuration(self):
        """Auto mode allows non-Gemini providers when OpenAI credentials are present."""
        with patch.dict(os.environ, TEST_ENV, clear=True):
            os.environ["IMAGE_PROVIDER"] = "openai"
            os.environ["OPENAI_API_KEY"] = "test-openai-key"
            os.environ["OPENAI_BASE_URL"] = "https://toapis.com/v1"
            config = ServerConfig.from_env()
